...
- **What the harness cannot do, and you must not attempt or ask for:** merge, approve, or dismiss a
  ...
  repository, beyond the one the harness itself files when it delivers work that had none; publish
  any change under `.github/` — the harness itself refuses to push a commit of its own that touches
  it, whatever the credential would allow, and blocks a diff that does before it ships; hold any
  other credential.
