...
**To give it work, comment `/harness work <what, or a link>` on the inbox issue**, or `@jgoetzmann-bot /harness work` on the brightboost issue itself. The inbox is heard within minutes, brightboost within three hours on a weekday, and the first row above is what you will see. Assigning `@jgoetzmann-bot` rarely helps: GitHub offers it as an assignee only on an issue it has already commented on (D68), and nearly every such issue already has a work item, which assignment leaves alone — it works only where the bot answered without opening one, such as after an `ask`. Everything else on this page is what to do once it has.
...
So `/harness revise` on an upstream PR at 14:00 UTC on a Friday is acted on around 15:41 that day, and at 20:00 Friday around 21:41; the same comment at 22:00 Friday waits until about 00:41 Monday, because the cron does not run at weekends. To skip the wait, run `feedback.yml` by hand — or, without access to this repository, post any `/harness` command on the inbox, which wakes the same job and its sweep reads the notifications too.
...
**Assign `@jgoetzmann-bot` to an issue on brightboost** that it has already commented on. GitHub offers an account as an assignee only when it is a collaborator, an organisation member or already in the thread, and on brightboost the bot is only ever the third (D68) — so on a fresh issue this gesture is not available; comment `@jgoetzmann-bot /harness work` there, or `/harness work <link>` on the inbox. Even where it is available it rarely adds anything: nearly every issue the bot has commented on already has a work item, which the sweep leaves alone, so assignment helps only where the bot answered without opening one, such as after an `ask`. `feedback.yml` sweeps for assigned issues every three hours on a weekday and opens a work item for each, so you do not have to touch this repository at all; it is idempotent, so an issue already queued is left alone. No model call — which issues are assigned is a fact, not a judgement.
...
`harness-ok` is **the pool**: the issues a maintainer has marked as fair game, by adding the label on brightboost. No command is involved, and removing the label takes an issue back out. The label exists, and a first batch of about forty issues, mostly `pod: build`, is being labelled. One model call ranks what survives, at most `SUGGEST_MAX_PER_RUN` (5) become `via:suggested` items, and the same run proposes each and comments once on its brightboost issue asking for a green light. **Every suggestion still waits for gate 1, and your merge is what builds it**, whether or not a maintainer answered: `implement.yml` approves a merged proposal without looking for a green light. A maintainer's yes is a plain comment on the proposal PR; a no is `/harness stop` there, which parks the item, and B332 keeps the issue out of later triage. `/harness go` is not a yes to rely on: on an unmerged suggestion it approves an item implement has no plan for, and every in-window build run — `implement`'s and the sweep's `harness run` — fails looking for one until you merge (D68). A shut or empty pool makes no model call. `ignore_allowlist: true` drops the label filter for a run; the other three still apply.
