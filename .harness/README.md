...
| `config.json` | Operational knobs. The allowed keys are `config.CONFIG_JSON_KEYS`, twenty-one of them; any other key is a startup error naming it. A value here overrides the same key in `.env`. | a PR |
...
how many delivery pull requests may be open upstream at once, whether it may comment upstream,
the fork and upstream names, the inbox and tracking issues, and the trust file path.
