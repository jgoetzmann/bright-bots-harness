...
| `config.json` | Operational knobs. The allowed keys are `config.CONFIG_JSON_KEYS`, seventeen of them; any other key is a startup error naming it. A value here overrides the same key in `.env`. | a PR |
...
comment upstream, the fork and upstream names, the inbox and tracking issues, and the trust file
path. [docs/OPERATIONS.md](../docs/OPERATIONS.md) explains the usage stop and the run window, and
how to change them.
